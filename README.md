...
This repository includes Xerces-C++ 3.3.0 sources under `third_party`, the CMake
recipe and the build script. CopyUSB is not required.

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\Build-Yafs.ps1 -Version 1.2.0
```

The result is `dist/1.2.0-x86` plus a ZIP containing the executable, schema,
licenses and matching sources. See [BUILD_WINDOWS.md](BUILD_WINDOWS.md) for
prerequisites, x64 builds and packaging an already-built release into another app.